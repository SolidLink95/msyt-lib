...
impl MainControl for Control3 {
  ...
  fn parse(header: &Header, buf: &[u8]) -> Result<(usize, Control)> {
    ...
    let field_1 = header.endianness().read_u16(&mut c).context( "could not read field_1")?;
    let field_2_len = header.endianness().read_u16(&mut c).context( "could not read field_2 length")?;
    let mut field_2 = vec![0; field_2_len as usize];
    c.read_exact(&mut field_2).context( "could not read field_2")?;
  ...
  fn write(&self, header: &Header, mut writer: &mut dyn Write) -> Result<()> {
    ...
    header.endianness().write_u16(&mut writer, self.field_2.len() as u16)
      .context( "could not write field_2 length")?;
