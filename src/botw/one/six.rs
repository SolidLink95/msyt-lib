...
impl SubControl for Control1_6 {
  ...
  fn parse(header: &Header, mut reader: &mut Cursor<&[u8]>) -> Result<Control> {
    ...
    let field_5 = header.endianness().read_u16(&mut reader).context( "could not read field_5")?;
  ...
  fn write(&self, header: &Header, mut writer: &mut dyn Write) -> Result<()> {
    ...
    header.endianness().write_u16(&mut writer, self.field_5).context( "could not write field_5")?;
