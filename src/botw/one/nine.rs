...
impl SubControl for Control1_9 {
  ...
  fn parse(header: &Header, mut reader: &mut Cursor<&[u8]>) -> Result<Control> {
    ...
    let payload_length = header.endianness().read_u16(&mut reader).context( "could not read length")?;

    reader.read_exact(&mut unknown_buf[..]).context( "could not read for unknown bytes")?;
    let unknown_1 = if unknown_buf == UNKNOWN {
      Some(unknown_buf)
    } else {
      reader.seek(SeekFrom::Current(-12)).context( "could not seek backward")?;
      None
    };
    ...
    for cstring in strings.iter_mut() {
      let field_1 = header.endianness().read_u16(&mut reader).context( "could not read cstring field_1")?;
      let str_len = header.endianness().read_u16(&mut reader).context( "could not read cstring length")?;
    ...
    let unknown_2 = if reader.get_ref().len() > reader.position() as usize + 12 {
      reader.read_exact(&mut unknown_buf[..]).context( "could not read unknown_buf")?;
      if unknown_buf == UNKNOWN {
        Some(unknown_buf)
      } else {
        reader.seek(SeekFrom::Current(-12)).context( "could not seek backwards")?;
        None
      }
    } else {
      None
    ...
    let mut field_6 = [0; 2];
    reader.read_exact(&mut field_6).context( "could not read field_6")?;
  ...
  fn write(&self, header: &Header, mut writer: &mut dyn Write) -> Result<()> {
    ...
    header.endianness().write_u16(&mut writer, payload_length as u16).context( "could not write length")?;

    if let Some(ref unknown_1) = self.unknown_1 {
      writer.write_all(&unknown_1[..]).context( "could not write unknown_1")?;
    }

    for string in &self.strings {
      header.endianness().write_u16(&mut writer, string.field_1).context( "could not write cstring field_1")?;
      ...
      header.endianness().write_u16(&mut writer, str_bytes.len() as u16).context( "could not write cstring length")?;
      writer.write_all(&str_bytes).context( "could not write cstring bytes")?;
    }
    ...
    if let Some(ref unknown_2) = self.unknown_2 {
      writer.write_all(&unknown_2[..]).context( "could not write unknown_2")?;
    }

    writer.write_all(&self.field_6[..]).context( "could not write field_6")?;
